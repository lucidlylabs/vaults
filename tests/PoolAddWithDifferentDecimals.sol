...
contract PoolAdd is Test {
    ...
    function setUp() public {
        ...
        // token0 = new MockToken("token0", "t0", 8);
        // token1 = new MockToken("token1", "t1", 8);
        // token2 = new MockToken("token2", "t2", 18);

        mrp = new SWBTCRateProvider();
        ...
        staking = new MasterVault(address(pool), "XYZ Mastervault Token", "XYZ-MVT", 200, jake, jake);
        ...
        deal(address(token0), alice, 100_000 * 1e18); // 100,000 SWBTCWBTC_CURVE
        deal(address(token1), alice, 100_000 * 1e8); // 100,000 SWBTC
        deal(address(token2), alice, 100_000 * 1e18); // 100,000 GAUNTLET_WBTC_CORE
    ...
    function testAddLiquidityInitial() public {
        uint256 total = 10_000 * 1e8; // 10000 WBTC worth of assets
