...
contract Vault is ERC4626Fees, Ownable {
    ...
    event PerformanceFeeSet(uint256 indexed performanceFeeInBps);
    event ManagementFeeRecipientSet(address indexed managementFeeRecipient);
    event PerformanceFeeRecipientSet(address indexed performanceFeeRecipient);
    ...
    function _accruePerformanceFee() internal {
        ...
        uint256 poolYield = totalAssetsValue - totalUserDeposits;

        if (poolYield > highWaterMark) {
            uint256 yieldSinceLastMark = poolYield - highWaterMark;

            if (yieldSinceLastMark > 0) {
                uint256 feeAmount = (yieldSinceLastMark * performanceFeeInBps) / 10_000;

                if (feeAmount > 0) {
                    accruedPerformanceFees += feeAmount;
                }

                highWaterMark = poolYield;
            }
        }
    }
